...
The player profile measures VPIP, PFR and exact preflop 3-bets, plus opportunity-based fold frequency, fold-to-3-bet, fold-to-flop-c-bet, flop c-bet, turn barrel, river aggression, check-raise, WTSD and W$SD. Rates with a specific decision opportunity include their denominator so Adaptive does not treat a tiny sample as conclusive.

Adaptive receives at most 25 compact completed hands. These contain public actions and line tags, not hole cards. Repeated tags such as `preflop-aggression+flop-pressure` are detected across that window. Both the OpenAI player and the offline DummyBot use the profile; the DummyBot applies sample-weighted changes to its defend, call, raise, fold, bluff and thin-value frequencies.
